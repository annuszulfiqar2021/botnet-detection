...
export P2P_DNLD_TARGET	= download_p2p_dataset.py
export DATASET_TARGET 	= process_dataset.py
export TF_DTST_TARGET 	= gen_tf_dataset.py
...
download-p2p:
	python $(P2P_DNLD_TARGET) --user $(MC17_USER) --server $(MC17_SERVER) --password $(MC17_PASSWORD) --remotedir $(P2P_REMOTE_DIR) \
...
gen-tf-dataset: | $(PROC_DATA_DIR)
	python $(TF_DTST_TARGET) --csvdir $(PROC_DATA_DIR)
