...
export CONV_TRAIN_DATA_DIR		= $(CONVERSATION_DATASET)/training
export CONV_EVAL_DATA_DIR		= $(CONVERSATION_DATASET)/evaluation
...
export CONVERSATION_DNN_NAME	= CONVERSATION_BOTNET
...
export CONVERSATION_FEATURES 	= num_pkts data_volume duration median_ipd isbotnet
export DNN_ARCHITECTURE 		= 64 64 64 64 64 64
export DNN_LAYER_PARS 			= 4 4 4 4 4 4
export INPUT_DIM 				= 19
export CONVERSATION_DIM			= 4
export N_CLASSES 				= 2
export OUTPUT_DIM				= 1
export METRIC 					= f1
...
.DEFAULT_GOAL = conversations-training
...
# 			--label isbotnet --input_dim $(INPUT_DIM) --output_dim $(N_CLASSES) --metric $(METRIC) --batch_size $(BATCH_SIZE) --epochs $(EPOCHS) \
# 			--model_dir $(MODELS_DIR) --spatial_dir $(SPATIAL_DIR)

# train-storm-model: | $(MODELS_DIR) $(SPATIAL_DIR)
# 	python $(TRAIN_TARGET) --name $(STORM_DNN_NAME) --arch $(DNN_ARCHITECTURE) --pars $(DNN_LAYER_PARS) --dataset $(PROC_DATA_DIR) --features $(MODEL_FEATURES) \
# 			--label isbotnet --input_dim $(INPUT_DIM) --output_dim $(N_CLASSES) --metric $(METRIC) --batch_size $(BATCH_SIZE) --epochs $(EPOCHS) \
# 			--model_dir $(MODELS_DIR) --spatial_dir $(SPATIAL_DIR)

# train-zeus-model: | $(MODELS_DIR) $(SPATIAL_DIR)
# 	python $(TRAIN_TARGET) --name $(ZEUS_DNN_NAME) --arch $(DNN_ARCHITECTURE) --pars $(DNN_LAYER_PARS) --trainset $(PROC_DATA_DIR) --evalset $(EVAL_DATA_DIR) \
# 			--features $(MODEL_FEATURES) --label isbotnet --input_dim $(INPUT_DIM) --output_dim $(N_CLASSES) --metric $(METRIC) --batch_size $(BATCH_SIZE) \
...
conversations-training: | $(MODELS_DIR) $(SPATIAL_DIR)
	python $(TRAIN_TARGET) --name $(CONVERSATION_DNN_NAME) --arch $(DNN_ARCHITECTURE) --pars $(DNN_LAYER_PARS) --trainset $(CONV_TRAIN_DATA_DIR) \
			--evalset $(CONV_EVAL_DATA_DIR) --features $(CONVERSATION_FEATURES) --label isbotnet --input_dim $(CONVERSATION_DIM) --output_dim $(OUTPUT_DIM) \
			--metric $(METRIC) --batch_size $(BATCH_SIZE) --epochs $(EPOCHS) --model_dir $(MODELS_DIR) --spatial_dir $(SPATIAL_DIR) --is_conversation
...
clean-model:
	-rm -rf $(shell find ./$(MODELS_DIR) -name "$(BOT_DNN_NAME)*") $(shell find ./$(MODELS_DIR) -name "$(CONVERSATION_DNN_NAME)*")
